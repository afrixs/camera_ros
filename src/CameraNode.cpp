...
#include "rpicam_encoder/rpicam_encoder.hpp"
#include <algorithm>
...
private:
  ...
  std::shared_ptr<RPiCamEncoder> rpicam_encoder;
...
CameraNode::CameraNode(const rclcpp::NodeOptions &options) : Node("camera", options), cim(this)
{
  ...
  declare_parameter("use_rpicam_encoder", true);
  ...
  use_rpicam_encoder = get_parameter("use_rpicam_encoder").as_bool();
  if (use_rpicam_encoder) {
    ...
    rpicam_encoder = std::make_shared<RPiCamEncoder>(pub_image_ffmpeg);
  }
  else {
...
bool CameraNode::setCaptureEnabled(bool enabled) {
  if (enabled) {
    ...
    if (camera->start(&controls_)) {
      ...
      if (rpicam_encoder)
        rpicam_encoder->setEnabled(true);
      return true;
    }
  }
  else {
...
bool CameraNode::stop() {
  ...
  if (rpicam_encoder)
    rpicam_encoder->setEnabled(false);
  // stop camera
  return !camera->stop();
}
...
CameraNode::process(libcamera::Request *const request)
{
  while (true) {
    ...
    if (request->status() == libcamera::Request::RequestComplete) {
      ...
      bool publish_image = (!pub_image_pure && pub_image.getNumSubscribers() > 0) || (pub_image_pure && pub_image_pure->get_subscription_count() > 0);
      bool publish_rpicam_ffmpeg = pub_image_ffmpeg && pub_image_ffmpeg->get_subscription_count() > 0;
      ...
      if (format_type(cfg.pixelFormat) == FormatType::RAW) {
        ...
        if (publish_rpicam_ffmpeg) {
          rpicam_encoder->encodeMessage(buffer, hdr, buffer_info[buffer], cfg);
        }
      }
        ...
        if (publish_rpicam_ffmpeg) {
          RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000, "RPiCam FFMPEG encoding not supported for compressed images");
        }
      }
      else {
